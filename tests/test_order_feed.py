...
import logging
import re
from helpers import is_order_in_progress
...
class TestOrderFeed:
    ...
    def test_open_order_details(self, browser, create_user):
        ...
        assert order_feed_page.is_element_visible(locators.ORDER_DETAILS_POP_UP), "Не удалось открыть детали заказа"
    ...
    def test_all_time_completed_counter_increases(self, browser, create_user):
        ...
        main_page.click_order_feed_link()
        assert order_feed_page.is_element_visible(locators.ALL_TIME_COMPLETED_COUNTER), "Не отображается счетчик 'Выполнено за всё время'"
    ...
    def test_completed_today_counter_increases(self, browser, create_user):
        ...
        main_page.click_order_feed_link()
        assert order_feed_page.is_element_visible(locators.COMPLETED_TODAY_COUNTER), "Не отображается счетчик 'Выполнено за сегодня'"
    ...
    def test_order_appears_in_progress(self, browser, create_user):
        ...
        assert is_order_in_progress(browser, order_number, locators.SECTION_IS_IN_PROGRESS), f"Заказ с номером {order_number} не найден в списке 'В работе'"